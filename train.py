...
def parse_arguments():
    p = argparse.ArgumentParser(description="Hyperparameters")
    p.add_argument("-epochs", type=int, default=50)
    p.add_argument("-batch_size", type=int, default=32)
    p.add_argument("-learning_rate", type=float, default=0.0001)
    p.add_argument("-gradient_clip", type=float, default=10.0)
    return p.parse_args()
    ...
    with torch.no_grad():
        ...
        for b, batch in enumerate(val_iter):
            ...
            loss = F.nll_loss(output[1:].view(-1, vocab_size),
                              trg[:1].contiguous().view(-1),   # use reshape??
                              ignore_index=pad)
            total_loss += loss.data.item()
        return total_loss/len(val_iter)


def train(e, model, optimizer, train_iter, vocab_size, gradient_clip, DE, EN):
    model.train()
    total_loss = 0
    pad = EN.vocab.stoi['<pad>']
